    ...
    public class UDPServer // udp 소켓 생성 및 수신한 패킷 처리
    {
        ...
        private void OnReceive(IAsyncResult ar) // 비동기적으로 수신된 패킷을 처리하는 콜백 함수, ar = BeginReceiveFrom() 호출로 부터 반환된 객체
        {
            try
            {
                ...
                Client.PacketDatagram packet = PacketSerializer.Deserializer(buffer) as Client.PacketDatagram; // 수신한 패킷을 역직렬화
                if (packet != null)
                {
        ...
        private void HandlePacket(Client.PacketDatagram packet, IPEndPoint remoteEP) // 수신한 패킷을 출력 & 추후 처리
